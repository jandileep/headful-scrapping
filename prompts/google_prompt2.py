...
JSON_FILE_PATH = "output_content.json"
...
try:
    ...
    if not images_info:
        print(f"No 'images' array found in {JSON_FILE_PATH} or it is empty.")
        exit()
...
for image_detail in images_info:
    local_path = image_detail.get("local_path")
    caption = image_detail.get("caption", "No caption provided.")

    if not local_path:
            print(f"Skipping entry due to missing 'local_path': {image_detail}")
    continue

    print(f"\n--- Processing image: {local_path} ---")
...
# image = Image.open("/root/headful-scrapping/Food_and_Culture/Cuisines_of_India/Central/The_Food_of_Maharashtra/images/maharashtrian-12_0.jpg")
        ...
        * **Culturally Precise Terminology:**
        ...
        1.  **Image:** A visual image depicting a specific aspect of Indian culture.
        ...
        **Key Guidelines for Task 1:**
        ...
        2.  **Profound Cultural Depth & Specificity:**
        ...
        3.  **Extreme Complexity & Higher-Order Thinking:**
        ...
        4.  **Precision & Unambiguity:** Despite their difficulty, questions must be clear, unambiguous, and meticulously worded to point towards a single best answer derivable from a deep and combined interpretation of the image, caption, and full text.
        ...
        **Image:**
        [You will place the image content/path here when using this prompt]
        ...
        **Full Text:**
        ...
        print(f"Error: Image file not found at {local_path} (double check after os.path.exists).")
    except PIL.UnidentifiedImageError:
        print(f"Error: Cannot identify image file. It might be corrupted or not a valid image: {local_path}")
    except Exception as e:
        print(f"An error occurred while processing {local_path}: {e}")
    finally:
        if 'image' in locals() and hasattr(image, 'close'):
            image.close() # Good practice to close the image file
