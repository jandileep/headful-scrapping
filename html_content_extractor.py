...
class HtmlContentExtractor:
    ...
    def _extract_content_from_html(self, soup, title):
        ...
        # --- NEW: Extract any og:image (or twitter:image) metas up front ---
        meta_images = []
        # Open Graph images
        for meta in soup.find_all('meta', property='og:image'):
            if meta.get('content'):
                meta_images.append({
                    "url": urljoin(self.url, meta["content"]),
                    "alt_text": "",
                    "title": "",
                    "caption": "og:image",
                    "paragraph": ""
        ...
        # Extract paragraphs and inline images together
        paragraphs, images_with_paragraphs = self._extract_content_with_context(main_content)
        
        # Prepend any meta images so they get downloaded too
        images_with_paragraphs = meta_images + images_with_paragraphs
    ...
    def _process_image_element(self, img, current_paragraph, images_with_paragraphs, figure_element=None):
        ...
        if src:
            ...
            if not caption and alt_text and len(alt_text) > 5:
                caption = alt_text
    ...
    def _extract_image_caption(self, img, figure_element=None):
        ...
        if parent:
            for caption_class in ['caption', 'wp-caption-text', 'figcaption', 'image-caption', 'storycaption']:
                ...
                if next_sibling and (next_sibling.has_attr('class') and any(c in caption_class for c in next_sibling.get('class', []))):
